...
-- 幂等，可重复执行。GaussDB/openGauss 兼容（run_source/task_origin 非保留字）。
-- 发布顺序：本文件 -> sql/slices/alerts.sql -> 更新后的 core.sql -> 后端 -> 前端。
ALTER TABLE sre_agent_run ADD COLUMN IF NOT EXISTS run_source text NOT NULL DEFAULT 'user';
COMMENT ON COLUMN sre_agent_run.run_source IS '会话来源：user=用户交互 / alert=告警自动诊断；会话历史列表默认排除 alert';
