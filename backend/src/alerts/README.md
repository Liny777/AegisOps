...
5. **终态通知 v3**（2026-08-19）：completed 与 failed 均 WeLink 通知 owner（`SEND_WELINK_MESSAGE_URL` +
   `OPENOPS_WEB_BASE_URL`；未配=不发）。completed 结论=根因结论（只取 rca.conclusion 截 200 字，
   ...
   内网样本反馈）。行格式（2026-08-21 验收）：告警标题/告警开始时间/命中策略/结论**各一行**
   （「接管结果」判定行已删——清单页该列不受影响）。全链日志：grep `[alerts][notify]`（含 outcome=），logger `openops.welink` 三态。
