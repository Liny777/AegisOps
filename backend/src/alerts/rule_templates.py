...
# ⚠ 改这段文本前先读 matcher.prompt_hash：它的内容哈希就是 incident.prompt_group，动了会让存量
#   未完结单的组标对不上（同一告警多开姊妹单、组冷却失效）；且前端新建规则时把本文原样预填并
#   落库，存量规则存的是副本、改这里覆盖不到它们。**平台级的通用要求（如输出语言）请加在
#   alerts/dispatcher.py 的 _build_prompt**——那里对默认/副本/自定义提示词一视同仁且不参与哈希。
...
要求：以只读诊断为主；过程结论同步到诊断面板；诊断收尾（update_diagnosis_board 的 step=5）时随 conclusion 一并提交 verdict——故障已恢复/已自愈填 recovered，需人工介入处理填 escalated，无法判定才留空（该值显示为告警清单的「接管结果」）；conclusion 用纯文本不要 Markdown 标记、第一句先讲最可能根因（前 200 字会作为通知摘要直接推送给用户）；关键结论与证据写入对话正文（不要只落沙箱文件）；信息不足以定界时，明确说明缺失的数据与建议的人工检查项。"""
