...
body={"userid", "data"}，响应 code==200 为成功）。fail-safe 铁律：任何失败只留
error 日志绝不抛——通知是锦上添花，不允许影响诊断收割主流程。
