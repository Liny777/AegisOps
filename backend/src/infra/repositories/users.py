...
async def get_user(user_id: str) -> dict[str, Any] | None:
    return await q_one(
        "select * from sre_openops_user where user_id=%(u)s and deleted_at is null", {"u": user_id}
    )
...
async def upsert_user(user_id: str, display_name: str, role: str = "user") -> None:
    await exec1(
        """
        insert into sre_openops_user (user_id, role, display_name, status, created_by, last_updated_by)
...
async def set_role(user_id: str, role: str, by: str) -> int:
    return await exec1(
        "update sre_openops_user set role=%(r)s, last_updated_by=%(b)s, last_update_date=now() where user_id=%(u)s",
