"""sre_platform_runtime_config 仓储：沙箱容量配置 + 平台模型注册（config_domain 区分）。"""
...
DOMAIN_MODEL = "platform_model"  # 已废弃（B7 起模型迁 sre_model_asset 表），保留常量供旧数据语义追溯


async def get_domain(domain: str) -> list[dict[str, Any]]:
    return await q_all(
        """
        select * from sre_platform_runtime_config
...
async def upsert(domain: str, key: str, value: Any, *, value_type: str = "json",
    ...
    row = await q_one(
        """
        select config_id from sre_platform_runtime_config
    ...
    if row:
        await exec1(
            """
            update sre_platform_runtime_config
        ...
        await exec1(
            """
            insert into sre_platform_runtime_config
