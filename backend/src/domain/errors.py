...
class Err:
    ...
    SKILL_PACKAGE_INVALID = "SKILL_PACKAGE_INVALID"  # 包结构不完整：entrypoint 指的脚本不在包内/既无入口也无手册
    SKILL_NAME_CONFLICT = "SKILL_NAME_CONFLICT"  # SkillHub 29.9：2003 他人已发布 / 2004 跨作用域同名——需改 SKILL.md 的 name 重传
    IAM_UPSTREAM = "IAM_UPSTREAM"  # B9：IAM 服务不可达/异常（502，可重试）
    OMODEL_AUTH_FAILED = "OMODEL_AUTH_FAILED"
    OMODEL_UPSTREAM = "OMODEL_UPSTREAM"
    ALERT_INCIDENT_STATE_INVALID = "ALERT_INCIDENT_STATE_INVALID"  # 告警聚合单状态不允许该动作（如忽略非排队单）
...
_STATUS = {
    ...
    Err.SKILL_PACKAGE_INVALID: 400,
    Err.SKILL_NAME_CONFLICT: 409,
    Err.IAM_UPSTREAM: 502,
    Err.OMODEL_AUTH_FAILED: 502,
    Err.OMODEL_UPSTREAM: 502,
    Err.ALERT_INCIDENT_STATE_INVALID: 409,
