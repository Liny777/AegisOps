...
def test_ddl_001_has_core_tables_plus_runtime_config():
    ...
    assert len(tables) == 36  # 24 业务核心（含 model_template 两表 + 2026-08-30 四号校验表） + runtime_config + P 块三表 + D 块 delegation + studio span（切片）+ alerts 六表（切片；订阅表 2026-08-15 随实例总开关下线）
