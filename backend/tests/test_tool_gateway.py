...
async def test_tool_005_user_mcp_no_platform_headers(quiet_emit):
    ...
    assert headers == {}  # 用户支路 headers 恒空字典（含 x-ec2-ip 亦不带）
...
def test_tool_012_platform_headers_carry_ec_ip(monkeypatch):
    """平台 MCP 出站带 x-ec2-ip = **后端主机自身 IP**（scope_ctx 无关；未配置即不发=零回归）。"""
    st = _st({})
    monkeypatch.setenv("OPENOPS_EC2_IP", "10.7.7.7")
    assert tool_gateway._platform_headers(st, _RUN)["x-ec2-ip"] == "10.7.7.7"

    monkeypatch.delenv("OPENOPS_EC2_IP")
    assert "x-ec2-ip" not in tool_gateway._platform_headers(st, _RUN)


def test_tool_013_ec_ip_is_not_the_browser_client_ip(monkeypatch):
    """锁住两个 IP 的语义区分：x-ec2-ip=后端主机，IAM-Client-Ip/X-Forwarded-For=用户浏览器。
