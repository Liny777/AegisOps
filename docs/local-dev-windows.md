...
8. **Windows 事件循环:必须用 `SelectorEventLoop`**(已由 `backend/run.py` 处理,`run-backend.sh` 走它)。原因:psycopg3 的**异步连接池不支持 Windows 默认的 `ProactorEventLoop`**(报 `Psycopg cannot use the 'ProactorEventLoop'...`)。`run.py` 的做法是**显式建 `SelectorEventLoop` 并在其上 `await server.serve()`**,而不是只设全局 policy——因为 `uvicorn.run()`/`Server.run()` 内部会自己 `asyncio.run(...)` 新建 loop,某些 uvicorn/Python 组合下并不落到我们设的 policy 上(实测仍是 Proactor)。所以**别用 `uvicorn main:app` 直接起**(那样是 Proactor→连库失败),用 `python run.py`(或 `run-backend.sh`)。启动日志会打印 `[OpenOps] event loop: ...SelectorEventLoop` 供一眼确认。副作用:SelectorEventLoop 不支持子进程→沙箱 `run_skill/run_bash` 在 Windows 更加不可用,但本就(无 `sh`)不可用、mock 调试不碰,可接受。`conftest.py` 同款守护(policy 方式,对 TestClient/anyio 足够),Windows 上 pytest 也走 selector。
