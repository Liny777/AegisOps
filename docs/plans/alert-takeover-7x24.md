...
- **按 prompt 分单**（2026-08-19 拍板：提示词不同的命中规则分开处理，支持 A/B 对比）：同实例多规则命中同一告警时，按归一 prompt（空白≡系统默认；`matcher.effective_prompt`）分组，**每组各建一单各自诊断**，归一相同合并一单；`sre_alert_incident.prompt_group`（归一 prompt 哈希前 12 位）参与附着/冷却/窗口补路由判定，NULL=分单上线前存量单按通配兼容（旧未完结单整体附着、旧完结单冷却全组）。规则加载 `list_enabled_rules` 加 ORDER BY（最老优先）消除「哪条生效」的行序漂移。**计数口径变化**：queued/skipped/stale/attached 计数从「按告警×实例」变「按告警×实例×prompt 组」；每多一个 prompt 组=多一次诊断 run+一条完成通知（文案带命中规则名以区分姊妹单），实例上限 10/全局 50 为总量闸被组共享。已明示不做：组级原子 admission（触顶时可能一组入队一组留痕，留痕可 retry）、派发期组间去重（排队期改 prompt 可能出现两组同 prompt 的重复诊断）。**清单姊妹单全量透出**（2026-08-20 测试反馈：清单行 LATERAL 只投影 top-1 单，第二条策略的诊断在页面无入口，被当成「同一条告警只跑了一次」）：`GET /alerts/events` 行加 `takeovers[]`（页内二次查询 `takeovers_by_events`，与 LATERAL 同视角过滤同排序，首元素恒等于投影单；不并进主查询——count 刚做过去 LATERAL 性能治理，且 jsonb_agg 无 GaussDB 先例），前端接管策略列逐条展示各姊妹单（策略名可点开各自会话 + 状态短标），操作列多会话时显「查看处理会话（N）」。行级 matched_rule/run_id 等旧字段口径不变（历史预览行/存量后端无 takeovers 时前端回落旧口径）。
- **派发前双闸**（存量 queued 单的迟到撤回）：匹配面（`list_enabled_rules`）每批重读、改动即时生效，但队列里已入队的单不受影响——`dispatch_once` 取到队首后再过两道，任一不过即置 `skipped` 留痕、不占诊断名额。① 白名单闸（属主排队后被移出开通名单 → `grant_revoked`）；② **规则闸**（2026-08-20：命中规则被停用/软删 → `rule_revoked`）——此前存量单照样起 run 跑完并发通知，最长要拖到 `alert_queue_max_age_s`（默认 1 天）排队超时才翻 failed，与用户点「停用」的直觉预期相悖。两闸口径：**全灭才拦**（同 prompt 组还剩一条存活且启用的规则就照常派发，与派发取提示词的回落口径同源 `matcher.rule_candidates`）+ **候选为空 fail-open**（存量单两个规则字段皆空时放行，不误杀历史数据）；`:retry` 走同一判定前置守卫（不挡在服务层，重试出来的单会被派发闸瞬间弹回 skipped，像按钮坏了）。已明示不做：不打断 `diagnosing` 中的单（要走 `cancel_task_state`，风险面大得多且只剩半截会话）、不在规则写路径同步清队（靠派发闸兜底，天然覆盖「停用后又改回来」与「清队/入队并发」两种竞态）。
...
- **防风暴七层**：拉取背压(200×5批) → 指纹去重 → 附着聚合 → 组冷却 → 有界队列(实例 10/全局 50，低严重度先丢，skipped 留痕) → 并发闸(**峰值模型并发=闸×(1+max_children=3)，闸 2 即 8，上线初期设 1**) → 两级开关(DB alert_enabled ＞ 规则；实例订阅维度 2026-08-15 已下线)。
