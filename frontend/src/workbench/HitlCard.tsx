      ...
      <div className="oa-hitl-card-grid" style={{ display: "grid", gap: 12, padding: "15px 16px" }}>
        ...
        <div className="oa-hitl-card-content" style={{ minWidth: 0 }}>
          <div className="oa-hitl-heading" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span className="oa-hitl-title" style={{ fontSize: 13.5, fontWeight: 700 }}>{hitl.title}</span>
            <span className="oa-hitl-tool" style={{ fontSize: 11, fontWeight: 600, color: color.brandStrong, background: "#fff", border: `1px solid ${color.brandTintBorder}`, padding: "2px 8px", borderRadius: radius.sm, fontFamily: "ui-monospace, monospace" }}>{hitl.tool}</span>
            {status === "pending" ? <span className="oa-hitl-countdown" style={{ fontSize: 11, color: tc.text }}>剩余 {hitl.countdown}</span> : null}
          </div>
          <div className="oa-hitl-summary" style={{ fontSize: 13, color: color.textBody, lineHeight: 1.55, marginTop: 6 }}>{hitl.summary}</div>
          ...
          {status === "pending" ? (
            <>
              <textarea
                className="oa-hitl-reason"
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="确认理由（可选）"
                rows={2}
                style={{ width: "100%", marginTop: 12, border: `1px solid #dfe2e8`, borderRadius: radius.md, padding: "8px 11px", fontSize: 12.5, lineHeight: 1.45, outline: "none", background: "#fff" }}
              />
